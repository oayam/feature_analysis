...
declare interface NavigationOptions {
  ...
  * <br>**true**: Display the page in full screen overlay mode.
  * <br>**false**: Display the page normally.
  *
  * @type { ?boolean }
  * @syscap SystemCapability.ArkUI.ArkUI.Full
  * @crossplatform
...
this.pageStack.pushPath(
  ...
  {
    fullScreenOverlay: false // fullScreenOverlay默认为false
  });
```
...
如果当前页面为dialog类型，与普通页面动画行为保持一致：
1. 如果关闭了动画：不做动画
2. 如果开启了动画，且有设置自定义动画：做自定义动画
3. 以上都不满足：做dialog的默认动画

#### 4.5.2 standard类型页面动画

如果当前页面为standard类型：
1. 如果关闭了动画：不做动画，一帧切换页面
2. 如果开启了动画，且有设置的自定义动画：做自定义动画
3. 以上都不满足：做SLIDE_RIGHT类型的动画（注意：不是SLIDE_BOTTOM！）

##### 4.5.2.1 分栏显示跳转到全屏覆盖页面
...
3. 在全屏模式下，需要注意分割线（divider）的显示层级。divider的显示层级不能比全屏页面高。
4. 当standard类型页面的fullScreenOverlay属性发生变化时，需动相应的调整SLIDE_RIGHT动画标志位。

## 六、变更记录

### 6.1 v1.1

补充有关分割线的实现要点（5.2中的第三点）。

### 6.2 v1.2

1. 变更3.1.1章节，从`扩展NavPathInfo接口`变为`扩展NavigationOptions接口`，具体的实现做对应的修改。
2. 变更[全屏判断规则](#42-全屏判断规则)的示例代码为CPP代码。

### 6.3 v1.3

1. 变更4.5章节，补充了更详细的动画规格。
2. 对应的，补充了5.2章节的第4点。
